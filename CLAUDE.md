...
- **`src/services/tmuxService.ts`** — `execSync` wrappers for tmux commands (session, window, pane CRUD). All names/paths are single-quote-escaped before shell interpolation. Contains `PANE_LAYOUTS` (9 preset layouts with preview ASCII art and split instructions) and `initPanes()` which executes sequential `split-window` commands. Also exports `formatTime()` for epoch-to-readable conversion.
  ...
  - `saveTmuxSessions(pluginsDir)` — runs tmux-resurrect's `save.sh` via `tmux run-shell -b` (tmux server-side background), returning `true` when a save was started; called after `createSession` so new sessions survive a restart. `-b` is required: plain `run-shell` blocks until the full snapshot (all sessions/panes, serially) finishes — 30s+ on large environments. The `init` command surfaces this with step output (`✔ Session created` / `⏳ Saving tmux snapshot in background`).
